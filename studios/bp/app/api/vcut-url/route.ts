/** Resolves VCut's real origin for the Create page's `<iframe>` (see
 *  `app/projects/[id]/create/page.tsx`) — VCut is now its own standalone app (`studios/vcut`),
 *  not something bp hosts in-process.
 *
 *  `http://localhost:3002` matches VCut's own `pnpm dev` port, exactly the same "hardcoded known
 *  dev port" convention `packages/universe/src/constants.ts`'s `CELESTIAL_BODIES.launchUrl`s already
 *  use for bp (3001) and gamedev (5173). In the packaged desktop app, VCut's server is spawned on
 *  a dynamically-chosen loopback port instead — `apps/desktop/src/main.ts` passes the real resolved
 *  URL to bp's own spawned process as `VCUT_URL`, read here.
 *
...
export async function GET() {
  return Response.json({ url: process.env.VCUT_URL ?? "http://localhost:3002" });
}
