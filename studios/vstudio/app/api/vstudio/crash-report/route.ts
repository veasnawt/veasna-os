...
/** Web-mode counterpart to `apps/vstudio-desktop/src/main.ts`'s own `logCrash` — the browser (whether
 *  the standalone `/` page or BP Studio's `<iframe>` embed of `/edit`) has no filesystem access to
 *  write a log itself, so `packages/vstudio/src/api/crashLog.ts`'s `reportError` POSTs here instead
...
export const POST = localRoute(async (req) => {
  ...
  try {
    fs.mkdirSync(VSTUDIO_ROOT, { recursive: true });
    const logPath = path.join(VSTUDIO_ROOT, "crash.log");
    ...
    console.error("[vstudio] failed to write crash log:", err);
  }
