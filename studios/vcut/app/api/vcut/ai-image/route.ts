...
/** `POST /api/vcut/ai-image?projectId=...` `{prompt, aspectRatio, model, deliverBytes}` — synchronous,
 *  unlike Remove Object/Auto Captions' own job+SSE pattern: every model here typically finishes in low
 *  single-digit seconds, so holding one HTTP request open for the whole thing is simpler than a job
 *  map/polling loop for a real latency this short. AI VIDEO generation (`ai-video/route.ts`) is the
 *  opposite case — that one takes minutes, and uses the job/SSE pattern for exactly the reason this
 *  route doesn't need it.
 *
 *  CORS-enabled (`hostedCreditGatedRouteCors`, always requires a session — see its own doc comment):
 *  desktop and mobile call this directly on the live vcut.io deployment now, same as Stock/Stickers
 *  search, but generation itself (unlike those two) has no "download a public CDN URL yourself" escape
 *  — the whole call needs the secret Replicate token, so there's no way to keep any part of it local.
 *  `deliverBytes: true` is what those callers set: the generated bytes ride back in THIS response
 *  (`bytesBase64`) instead of only being saved into vcut.io's own per-user library, so desktop/mobile
 *  can import them into the user's actual local project in one round trip rather than a second fetch
 *  back down. Ignored (bytes never included) for the hosted web app itself, which has no local
 *  filesystem of its own to save into anyway — the returned `asset`'s `relPath` is all it ever needed. */
export const POST = hostedCreditGatedRouteCors("ai-image", MODELS[DEFAULT_MODEL].credits, async (req, user, spend) => {
  ...
  try {
    ...
    if (VCUT_HOSTED && user) {
      ...
      return Response.json(body?.deliverBytes ? { asset, bytesBase64: bytes.toString("base64") } : { asset });
    }
 ...
 *  every other feature's own `HEAD` already draws (a 0-credit user still gets 204 here). CORS-enabled
 *  for the same reason `POST` is now. */
