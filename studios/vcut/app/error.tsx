...
import { reportError } from "@veasnawt/vcut";

/** Next's own App Router error-boundary convention — a second layer above the manual
 *  `ErrorBoundary` `VCutApp` wraps its own export in (`packages/vcut/src/ui/VCutApp.tsx`).
 *  Catches anything that somehow escapes that boundary, plus errors in this route's own page code
 *  BEFORE `VCutApp` even mounts (e.g. a future addition to `edit/page.tsx` throwing ahead of the
 *  `<VCutApp>` render). Reports through the exact same `reportError` the inner boundary uses, so
