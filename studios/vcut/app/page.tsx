...
function thumbnailUrl(projectId: string, thumbnail: ProjectSummary["thumbnail"]): string | undefined {
  ...
  return `/api/vcut/media/raw?projectId=${encodeURIComponent(projectId)}&relPath=${encodeURIComponent(thumbnail.relPath)}&kind=${thumbnail.kind}`;
}
...
function NewProjectDialog({ onClose, onCreated }: { onClose: () => void; onCreated: (project: Project) => void }) {
  ...
  async function submit() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/vcut/project", {
        method: "POST",
      ...
      if (media) {
        ...
        const importRes = await fetch(`/api/vcut/media?projectId=${encodeURIComponent(project.bpProjectId)}`, {
          method: "POST",
          body: form,
        });
        ...
        const saveRes = await fetch(`/api/vcut/project?projectId=${encodeURIComponent(project.bpProjectId)}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
...
/** VCut's own home — a real entry point now that it's a standalone app, not just something BP
 *  Studio's Create page hands a `projectId` to. Lists every project on disk (`/api/vcut/projects`)
 *  and lets a new one be started (`POST /api/vcut/project`, via `NewProjectDialog`) without any host
 *  app involved. A host app embedding VCut (BP Studio, today) still skips straight to
 *  `/edit?projectId=...` directly — this page is for when VCut is opened on its own. */
export default function Home() {
  ...
  async function deleteProject(project: ProjectSummary) {
    setDeleting(true);
    try {
      const res = await fetch(`/api/vcut/project?projectId=${encodeURIComponent(project.id)}`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error();
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-6xl flex-col gap-6 px-4 py-8 sm:gap-8 sm:px-6 sm:py-12">
      <header className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-lg font-semibold text-white">VCut</h1>
