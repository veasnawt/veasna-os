...
/** Forks the bundled standalone VCut Next.js server under Electron's own Node runtime (no
...
export async function spawnNextServer(extraEnv: Record<string, string>): Promise<{ url: string; stop: () => Promise<void> }> {
  ...
  if (!fs.existsSync(serverJsPath)) {
    throw new Error(`Bundled VCut server not found at "${serverJsPath}" — did "pnpm package" run?`);
  }

  // Same port VCut's own `pnpm dev` uses — see pickFreePort.ts's comment for why a stable port
  ...
  child.stdout?.on("data", (chunk: Buffer) => process.stdout.write(`[vcut] ${chunk}`));
  child.stderr?.on("data", (chunk: Buffer) => process.stderr.write(`[vcut] ${chunk}`));
