/**
 * Next.js App Router API route for Rixie Agent with persistent SQLite SessionStore.
 */
import { NextRequest, NextResponse } from "next/server";
import { RixieAgent } from "../../../core/agent/agent";
...
export async function POST(req: NextRequest) {
  try {
    const { message, provider, model, sessionId = "default_session", studio = "global" } =
      (await req.json()) as {
        message?: string;
        provider?: string;
        model?: string;
